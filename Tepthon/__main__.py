...
async def login(_: Client, callback: CallbackQuery):
     ...
     elif not users.get(user_id, {}).get("vip", False):return await callback.answer("- انتهت مدة الإشتراك الخاص بك.", show_alert=True)
     ...
     except exceptions.TimeOut:return await callback.message.reply(text="- نفد وقت استلام الجلسة", reply_markup=Markup([[Button("- العوده -", callback_data="account")]]))
